...
{
    public class HistoryTests : TestBase, IAuthenticatedUiTest
    {
        [Fact]
        public async Task Xem_Lich_Su_Don_Hang_Thanh_Cong()
        {
            // Arrange - Login
            ...
            await historyPage.NavigateToHistoryTabAsync();
            await Task.Delay(2000, TestContext.Current.CancellationToken);

            // Assert
            Page.Url.Should().Contain("dashboard");
            var rowCount = await historyPage.GetGridRowCountAsync();
            rowCount.Should().BeGreaterThanOrEqualTo(0); // Chỉ cần load được grid không lỗi
        }
    }
}
