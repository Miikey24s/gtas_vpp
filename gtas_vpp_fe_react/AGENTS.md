...
- Agent chạy trong Figma Make phải đọc thêm `Guidelines.md` và `../docs/design/VPP-PULSE-FIGMA-MAKE-BUILD-BRIEF.md`. Agent được tự quyết toàn bộ visual/UX và dựng thẳng bản xem trước để owner review; không bị ràng buộc bởi style cũ. Nếu chỉnh code thật, bắt đầu từ latest `origin/Nam` trên branch `figma/*` và không push trực tiếp vào `Nam`.
