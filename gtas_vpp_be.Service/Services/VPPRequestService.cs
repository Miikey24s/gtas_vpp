    ...
    public class VPPRequestService : BaseServices, IVPPRequestService
    {
        ...
        public async Task<VPP01_RequestHeaderResDTO> UpdateOrderAsync(VPP01_UpdateReqDTO req)
        {
            ...
            try
            {
                ...
                if (header.Status != (int)VPPStatus.Draft) throw new InvalidOperationException("Only Draft orders can be edited.");
                ...
                uow.VPPContext.Set<VPP02_RequestDetail>().RemoveRange(header.VPP02_RequestDetails);
                header.VPP02_RequestDetails = req.Items.Select(i => new VPP02_RequestDetail
                {
                    Id = Guid.NewGuid(),
                    VPPId = i.VPPId,
                ...
                await uow.CommitAsync();
                return (await GetOrderByIdAsync(req.Id))!;
            }
            catch
            {
                uow.Rollback();
        ...
        public async Task DeleteDraftAsync(Guid id, int userId)
        {
            ...
            try
            {
                ...
                if (header == null) throw new KeyNotFoundException("Order not found.");
                if (header.Status != (int)VPPStatus.Draft) throw new InvalidOperationException("Only Draft orders can be deleted.");
        ...
        private static VPP01_RequestHeaderResDTO MapToResDTO(VPP01_RequestHeader h) => new()
        {
            ...
            Items = (h.VPP02_RequestDetails ?? new List<VPP02_RequestDetail>()).Select(d => new VPP02_RequestDetailResDTO
            {
                Id = d.Id,
                Description = d.Description,
