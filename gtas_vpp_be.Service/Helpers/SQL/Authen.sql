...
ALTER PROCEDURE dbo.sp_Authen_Login @Param NVARCHAR(MAX)
AS
BEGIN TRY
    ...
    (
        SELECT *
        FROM [172.16.39.162].GTAS_MENU.dbo.tblUsers us (NOLOCK)
...
ALTER PROCEDURE dbo.sp_Authen_TabUser_UserList @Param NVARCHAR(max)
AS
BEGIN TRY
        ...
        (
            SELECT p04.Id,
                   ...
                   (
                       SELECT TOP 1
                              FullName
                       FROM [172.16.39.162].GTAS_MENU.dbo.tblUsers u (NOLOCK)
...
ALTER PROCEDURE dbo.sp_Authen_TabUser_SearchUser @Param NVARCHAR(max)
AS
BEGIN TRY
DECLARE @SearchText_TabUser_SearchUser NVARCHAR(MAX);
        ...
        (
            ...
            FROM [172.16.39.162].GTAS_MENU.dbo.tblUsers p01 (NOLOCK)
...
ALTER PROCEDURE dbo.sp_Authen_GetPermissionSinglePage @Param NVARCHAR(MAX)
AS
BEGIN TRY
...
ALTER PROCEDURE dbo.sp_Authen_CreateNewGroup @Param NVARCHAR(max)
AS
BEGIN TRY
...
ALTER PROCEDURE dbo.sp_Authen_CopyFromGroup @Param NVARCHAR(max)
AS
BEGIN TRY
...
ALTER PROCEDURE dbo.sp_Authen_Permission_GetPageWithComponentByGroupId @Param NVARCHAR(max)
AS
BEGIN TRY
