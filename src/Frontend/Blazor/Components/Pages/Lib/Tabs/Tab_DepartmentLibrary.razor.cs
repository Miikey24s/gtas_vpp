...
public partial class Tab_DepartmentLibrary : IDisposable
{
    ...
    { try { var result = await ApiServices.PatchFromApiAsync<DepartmentResDTO>($"{Config.LibraryApi.Departments}/{row.Id}", new { IsDeleted = value, UpdatedAtUtc = DateTime.Now, UpdatedByUserId = Global.UserInfo.UserID }); if (result is null) { row.IsDeleted = !value; ToastService.Show(NotificationSeverity.Error, Loc["Error"], Loc["ChangeRecordStatusFailed"], 5000, true); return; } row.IsDeleted = result.IsDeleted; await grid.Reload(); } catch (Exception ex) { row.IsDeleted = !value; ToastService.Error(ex, Loc, "ChangeRecordStatusFailed"); } }
