...
|---|---|---|
| `src/Backend/Domain/VPP/VPP00_Period.cs` | new | Aggregate kỳ, UTC boundary, Vietnam timezone, state/audit/rowversion. |
| `src/Backend/Domain/VPP/VppPeriodState.cs` | new | Enum/state machine period. |
| `src/Backend/Domain/VPP/VPP01_RequestHeader.cs` | modified | Period, series/revision/lineage, supplement/cancel/idempotency/rowversion. |
| `src/Backend/Domain/VPP/VPP03_Log.cs` | modified | Typed actor/company/action/revision/correlation/reason audit. |
| `src/Backend/Domain/VPPMigrationDbContext.cs` | modified | EF mapping, lengths/checks/FK/indexes và filtered uniqueness. |
| `src/Backend/Application/Domain/PeriodCalculator.cs` | modified | Tính kỳ 05→04, local/UTC boundary đa nền tảng. |
| `src/Backend/Application/Domain/VppRequestPolicy.cs` | new | Config deadline/grace/quota/attempt limits và validation. |
| `src/Backend/Application/Helpers/Context/VPPContext.cs` | modified | Runtime EF mapping đồng bộ migration context. |
| `src/Backend/Application/Services/IVppPeriodService.cs` | new | Contract ensure/get/advance/transition idempotent. |
| `src/Backend/Application/Services/VppPeriodService.cs` | new | Tạo kỳ cạnh tranh an toàn, transition/recovery guard. |
| `src/Backend/Application/Services/VppPeriodRecoveryWorker.cs` | new | Hosted recovery/advance worker mỗi 300 giây. |
| `src/Backend/Application/Services/VPPRequestService.cs` | modified | Core create/update/cancel/history/supplement/approval, scope, concurrency, idempotency, period flags. |
| `src/Backend/Api/Controllers/VPPRequestController.cs` | modified | History endpoint; cancel envelope; approve/reject rowversion/idempotency/scope. |
| `src/Backend/Api/Program.cs` | modified | DI policy, period service, recovery worker. |
| `src/Backend/Api/appsettings.json` | modified | Explicit `VPP` config: day 5, grace 2, quota 3, attempts 6, recovery 300s. |

### 5.2 EF migration

| File | Trạng thái | Mục đích |
|---|---|---|
| `src/Backend/Migrations/Migrations/20260716063247_AddPeriodRequestRevisionAndSupplementWorkflow.cs` | new | Forward-only schema/data migration, preflight, legacy backfill, invariant checks, indexes/FK. |
| `src/Backend/Migrations/Migrations/20260716063247_AddPeriodRequestRevisionAndSupplementWorkflow.Designer.cs` | new | EF migration metadata. |
| `src/Backend/Migrations/Migrations/VPPMigrationDbContextModelSnapshot.cs` | modified | Snapshot model mới. |

### 5.3 Shared DTO/wire contracts

| File | Trạng thái | Mục đích |
|---|---|---|
| `src/Shared/DTOs/Req/VPP/ApproveOrderReqDTO.cs` | new | Rowversion/idempotency cho approve. |
| `src/Shared/DTOs/Req/VPP/VPP_CancelOrderReqDTO.cs` | new | Rowversion/reason/idempotency cho cancel. |
| `src/Shared/DTOs/Req/VPP/RejectOrderReqDTO.cs` | modified | Rowversion/idempotency reject. |
| `src/Shared/DTOs/Req/VPP/VPP01_CreateReqDTO.cs` | modified | Base request, supplement reason, idempotency. |
| `src/Shared/DTOs/Req/VPP/VPP01_UpdateReqDTO.cs` | modified | Supplement reason, rowversion, idempotency. |
| `src/Shared/DTOs/Res/VPP/VPP01_RequestHeaderResDTO.cs` | modified | Period/revision/lineage/supplement/audit/rowversion/CanReplace. |
| `src/Shared/DTOs/Res/VPP/VPP_PeriodInfoResDTO.cs` | modified | State/boundary/base/quota/attempt/eligibility reasons. |
| `src/Shared/DTOs/Res/VPP/VPP_RequestHistoryResDTO.cs` | new | Revisions và typed timeline. |

### 5.4 Frontend

| File | Trạng thái | Mục đích |
|---|---|---|
| `src/Frontend/Blazor/Components/Pages/VPPRequest/OrderCreateContext.cs` | modified | Wizard state cho base/reason/rowversion. |
| `src/Frontend/Blazor/Components/Pages/VPPRequest/OrderCreateStep2.razor` | modified | Reason riêng, validation 5–500. |
| `src/Frontend/Blazor/Components/Pages/VPPRequest/OrderCreateStep3.razor` | modified | Hiển thị reason lúc xác nhận. |
| `src/Frontend/Blazor/Components/Pages/VPPRequest/Page_OrderCreate.razor` | modified | Accessible steps, period/quota/deadline/base panel, submit guard. |
| `src/Frontend/Blazor/Components/Pages/VPPRequest/Page_OrderCreate.razor.cs` | modified | Eligibility, permission, replacement, payload/idempotency, safe errors. |
| `src/Frontend/Blazor/Components/Pages/VPPRequest/Tabs/BaseOrderTab.cs` | modified | Safe `UiErrorMapper`. |
| `src/Frontend/Blazor/Components/Pages/VPPRequest/Tabs/Tab_AdminApproval.razor` | modified | Per-row loading và approval UI. |
| `src/Frontend/Blazor/Components/Pages/VPPRequest/Tabs/Tab_AdminApproval.razor.cs` | modified | Approval/reject concurrency/idempotency và reason dialog. |
| `src/Frontend/Blazor/Components/Pages/VPPRequest/Tabs/Tab_History.razor` | modified | Action xem lifecycle. |
| `src/Frontend/Blazor/Components/Pages/VPPRequest/Tabs/Tab_History.razor.cs` | modified | History dialog/timeline loader. |
| `src/Frontend/Blazor/Components/Pages/VPPRequest/Tabs/Tab_Orders.razor` | modified | Eligibility/quota/period, replace/history/cancel CTA. |
| `src/Frontend/Blazor/Components/Pages/VPPRequest/Tabs/Tab_Orders.razor.cs` | modified | Guards, confirmation, rowversion/idempotency và safe errors. |
| `src/Frontend/Blazor/Components/Pages/VPPRequest/Components/Dialog_RejectSupplement.razor` | new | Reject reason dialog 5–500. |
| `src/Frontend/Blazor/Components/Pages/VPPRequest/Components/Dialog_RequestHistory.razor` | new | Loading/error/retry/timeline/revision dialog. |
| `src/Frontend/Blazor/Resources/Components.App.resx` | modified | Vietnamese strings kỳ/quota/revision/reason/action. |
| `src/Frontend/Blazor/Resources/Components.App.en.resx` | modified | English equivalents. |

### 5.5 Tests

| File | Trạng thái | Mục đích |
|---|---|---|
| `tests/Backend.UnitTests/Architecture/SharedContractSerializationTests.cs` | modified | Expected public JSON shape. |
| `tests/Backend.UnitTests/Architecture/SharedWireContractManifestTests.cs` | modified | Shared wire manifest hash (`453B8FADAA51AA680BF722AF867265C7BB34453F1BDEC54511C64F632BD5C634`). |
| `tests/Backend.UnitTests/Domain/VppPeriodPolicyTests.cs` | new | Defaults, limits, 05→05 boundary, grace, legal transitions. |
| `tests/Backend.UnitTests/Domain/VppPeriodServiceTests.cs` | new | Ensure idempotency, recovery và transition. |
| `tests/Backend.UnitTests/VPPRequestTests/CreateOrderAfterSettlementTests.cs` | modified | Settled chặn regular/supplement. |
| `tests/Backend.UnitTests/VPPRequestTests/CreateOrderRaceConditionTests.cs` | modified | Period/current revision race/index semantics. |
| `tests/Backend.UnitTests/VPPRequestTests/VPPRequestServiceTests.cs` | modified | Fixtures/typed audit/period assertions. |
| `tests/Backend.UnitTests/VPPRequestTests/VPPRequestLifecycleTests.cs` | new | Revision/history, cancel/replace, slot, stale rowversion, idempotency, scope/quota/self-decision. |
| `tests/Frontend.UnitTests/Services/ApiServicesJsonTransportTests.cs` | modified | Approval concurrency envelope serialization. |
...
dotnet test tests\Backend.UnitTests\gtas_vpp_be.Tests.csproj -c Release --no-build --no-restore
# PASS — 321 passed, 0 failed, 0 skipped

dotnet test tests\Frontend.UnitTests\gtas_vpp_fe.Tests.csproj -c Release --no-build --no-restore
# PASS — 87 passed, 0 failed, 0 skipped

dotnet test tests\Backend.IntegrationTests\gtas_vpp_be.IntegrationTests.csproj -c Release --no-build --no-restore
