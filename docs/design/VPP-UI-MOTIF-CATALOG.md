...
| `FILTER-TOOLBAR` | Search, filter, clear, column picker | Tìm/lọc dữ liệu thường xuyên | `VppDataToolbar`, `VppFilterSearch`, `VppFilterSelect`, `VppClearFiltersButton`, `VppColumnPicker` | filter count/domain-specific options; column picker trigger chỉ hiện số cột đang bật, tổng số nằm trong popup/aria | popup chrome, toolbar control height 32px, order |
