...
| `FILTER-TOOLBAR` | Search, filter, clear, column picker | Tìm/lọc dữ liệu thường xuyên | `VppDataToolbar`, `VppFilterSearch`, `VppFilterSelect`, `VppClearFiltersButton`, `VppColumnPicker` | filter count/domain-specific options; column picker trigger chỉ hiện số cột đang bật, tổng số nằm trong popup/aria; search tiếng Việt có/không dấu cho cùng kết quả | popup chrome, toolbar control height 32px, order |
...
- Cột mặc định phải ưu tiên dữ liệu giúp ra quyết định ngay: nhận diện, trạng thái, số bản ghi liên quan và độ mới quan trọng. Metadata tra cứu ít dùng như nguồn dữ liệu/lần đăng nhập, địa chỉ thành phần hoặc resource identifier vẫn `Pickable="true"` nhưng `Visible="false"`.
- Số liệu tổng hợp phải được tính từ dữ liệu đang hoạt động và có cùng phạm vi với grid; không dùng tổng đã xóa/vô hiệu hóa, không thêm cột rỗng chỉ để lấp khoảng trống. Cột tổng hợp không cho header filter nếu chưa có query filter tương ứng.
- Một khái niệm ghép có giá trị đọc nhanh như `Địa chỉ` được phép hiển thị một cột mặc định; các thành phần `Thành phố`, `Phường/Xã`, dòng địa chỉ phụ chỉ nằm trong picker. Không hiển thị đồng thời cả cột ghép và toàn bộ cột thành phần mặc định.
