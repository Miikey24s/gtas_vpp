...
| 2026-07-26 | Popover note editor in Order Create | Ghi chú chung của đơn giữ kiểu underline; ghi chú từng mặt hàng chuyển thành action `Ghi chú` có biểu tượng bút và mở popover chỉnh sửa theo motif ô lọc/Lịch sử đã duyệt | Owner nhận thấy chuỗi underline liên tiếp trong panel hẹp gây rối nhưng vẫn muốn giữ form underline cho ghi chú cấp đơn | M2.09 Order Create + shared order-line editor | Keep one underlined order-level note field; replace every repeated item underline with a compact keyboard-accessible trigger and one reusable anchored popover containing item context, editor and save/cancel actions | M2.09 Atlas now; map to accessible Blazor popover after approval | Implemented — OWNER_REVIEW |
