...
| 2026-07-29 | Chốt kỳ — unified data workspace | Dùng khung Catalog, bỏ KPI/readiness card và workflow bốn bước; hai selector `Kỳ này / Tùy chọn` + `Phòng ban / Mặt hàng`, mặc định Phòng ban. Dải quyết định gồm hai select canonical theo thứ tự `Nhà cung cấp → Bảng giá`, tổng tiền và CTA chốt kỳ; không còn footer/xem trước. Mặt hàng dùng snapshot demand + pager `100`, phòng ban dùng snapshot group | Owner muốn nhãn góc nhìn ngắn, bỏ tiền tố “Theo” và đặt Phòng ban trước Mặt hàng | Period settlement route | Tái dùng period-demand + preview/confirm/correct API; khóa popup page-size không tràn option khỏi panel; route-real desktop/tablet/mobile và human screenshot review | `PeriodSettlementPanel`, `VppFilterSelect`, Radzen dropdown bridge, settlement state | IMPLEMENTED — OWNER REVIEW |
...
| 2026-07-31 | My Orders action row + toast close | Dòng action của đơn hiện tại/bổ sung dùng cùng thứ tự `Lịch sử → PDF → Excel → Sửa/Hủy/Hoàn tác/Tạo lại → primary CTA`; nút đóng toast là icon phẳng, không border/radius oval, keyboard focus dùng underline inset | Owner phát hiện action cùng cấp bị tách rời và focus chrome cũ tạo oval nặng quanh nút đóng | Shared order workspace + global FEEDBACK | Sắp xếp lại `VppOrderWorkspacePanel`, để primary action cuối; xóa oval ở `vpp-toast.css`, giữ focus-visible không làm xê dịch. Architecture gate + computed-style visual QA | My Orders current/supplement/previous, History/export consumers, mọi Radzen toast | IMPLEMENTED — FOCUSED QA PASS |
