...
| 2026-07-30 | Admin CRUD placement | Toolbar quản trị chỉ còn search/filter/clear/column picker; nút Thêm nằm ở header cột Thao tác, còn Xem/Sửa/Xóa-Khôi phục và action vòng đời nằm trong cell Thao tác | Owner yêu cầu một vị trí thao tác duy nhất để không lẫn query controls với mutation controls | Local M6 + Library | Thêm `VppActionColumnHeader`, migrate Library/User/Permission group và guard không cho CRUD quay lại toolbar | `/library`, `/permission` | IMPLEMENTED — pending route-real visual approval |
