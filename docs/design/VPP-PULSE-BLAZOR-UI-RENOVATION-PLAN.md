...
| Quản lý người dùng | `IMPLEMENTED — QA PASS` | Nhóm quyền và phòng ban chỉnh trực tiếp bằng dropdown tại đúng cột; Radzen giữ tìm kiếm cho tập option dài nhưng dùng cùng height/radius/surface/popup/focus/disabled với filter canonical, không còn mảng nền con hoặc viền focus riêng. Bỏ action `manage_accounts`, dialog membership và model cũ để chỉ còn một luồng cập nhật. |
