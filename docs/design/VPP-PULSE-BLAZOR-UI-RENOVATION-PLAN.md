...
| 2026-07-22 | Optical vertical centring for primary tab labels | Label primary tab nhìn cân giữa theo thị giác thay vì chỉ cân line-box toán học | Owner vẫn thấy chữ thiên lên trên dù DOM đo top/bottom bằng nhau, do glyph system font có khối lượng thị giác tập trung phía trên baseline | Shared primary RadzenTabs typography | Dịch riêng `.rz-tabview-title` xuống 2px; không thay đổi header/button 72px, focus hitbox hoặc underline baseline | Dashboard, Library, Permission và mọi primary header-tab | Implemented — OWNER_REVIEW |
