...
| 2026-07-29 | Chốt kỳ — unified data workspace | Dùng khung Catalog, bỏ KPI/readiness card và workflow bốn bước; hai selector `Kỳ này / Tùy chọn` + `Theo đơn / Theo phòng ban`; toolbar chỉ search, loại đơn, trạng thái, phòng ban. Nhà cung cấp là rich dropdown ngay trong decision strip, mỗi lựa chọn hiển thị bảng giá, độ phủ và tổng tiền; không giả dạng filter, không cần nút so sánh/thay đổi rời; bảng đổi cột theo góc nhìn, đơn dùng server paging `100`, phòng ban dùng snapshot group, detail mở drawer overlay | Owner muốn Chốt kỳ gọn, đọc như một data workspace và yêu cầu bỏ chữ `Phương án chốt` | Period settlement route | Tái dùng preview/confirm/correct API; legacy period query đều vào workspace hợp nhất; route-real desktop/tablet/mobile và human screenshot review | `PeriodSettlementPanel`, `PeriodOperationsWorkspace`, settlement state, shared selectors/data frame/detail drawer | IMPLEMENTED — OWNER REVIEW |
