...
- Owner refinement 2026-08-14: cả 11 picker quản trị có thêm `ID hệ thống` của chính bản ghi, luôn ẩn mặc định; foreign-key ID, numeric `UserId` và `RowVersion` vẫn bị loại. Nhãn cột audit đổi thành `Ngày tạo`/`Ngày cập nhật`. Danh sách toàn bộ cột đang ẩn để duyệt nằm tại mục 8 của `UI-COLUMN-CONTRACT-001`.
