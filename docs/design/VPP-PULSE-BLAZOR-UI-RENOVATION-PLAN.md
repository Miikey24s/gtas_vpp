...
| Bước code hiện tại? | Data-surface DS0–DS2 đã được owner duyệt; DS3 đã chuẩn hóa Create Order, Department Summary và Period Review, đang chờ owner visual review. DS4/R1 vẫn khóa. | [Data-surface execution](../execution/UI-DATA-SURFACE-001.md#5-kế-hoạch-chi-tiết) |
...
| 2026-07-29 | Order Create virtual header paint guard | Cell Mặt hàng hai dòng không được ló tên/mã lên sticky header trong bất kỳ frame cuộn nào; các cột một dòng không phải bằng chứng lỗi đã hết | Guard cũ đợi tới animation frame kế tiếp nên vẫn có thể nháy một frame; ES module không versioned có thể bị browser giữ bản cũ khi dùng `dotnet watch` | Order Create catalog virtualization + module lifecycle | Cập nhật row visibility đồng bộ trong scroll event, tự nối lại khi Radzen thay scroll viewport, version module import và kiểm ảnh ở vị trí cuộn giữa danh sách | M2.09 Order Create | Implemented — VERIFIED |
