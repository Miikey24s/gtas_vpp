...
|---|---|---|---|---|---|---|---|
| 2026-08-13 | Các kỳ đặt hàng + grid compact | Mỗi dòng luôn có hai thao tác chính trực tiếp: `Xem` và `Chốt kỳ`; `Chốt kỳ` hiển thị disabled khi chưa đủ điều kiện. Menu `...` chỉ giữ `Gia hạn kỳ → Sửa lịch`; bỏ `Mở lại nhận đơn` và `Xóa kỳ` khỏi collection. Nếu cả hai thao tác phụ đều không khả dụng thì trigger `...` cũng disabled, nhưng vẫn giữ nguyên vị trí trong cột thao tác. DataGrid có ít bản ghi giữ chiều cao dòng compact; empty state vẫn lấp đầy và căn giữa | Owner muốn thao tác quan trọng nhìn thấy ngay, câu chữ ngắn hơn, không publish chức năng mở lại/xóa kỳ không còn dùng, và không mở một menu chỉ chứa các mục mờ. Grid Chốt kỳ ít dòng từng bị flex kéo cao gây sai mật độ design system | Global `DATA-ROW` + local `ADMIN-ROW-ACTIONS`, `CAPABILITY-SURFACE` | Đưa Xem/Chốt kỳ ra action trực tiếp theo thứ tự cố định; overflow chỉ giữ capability phụ thực sự còn dùng; dùng behavior mặc định của shared menu để disable trigger khi toàn bộ item disabled. Đổi flex table populated về content-sized, giữ override full-height riêng cho empty/filtered-empty | Mọi DataGrid canonical có ít dữ liệu; `OrderPeriodManagementWorkspace`, period route-real tests | IMPLEMENTED — QA PASS; build 0 warning/error; architecture `12/12`; route-real responsive `4/4` tại 390/768/1366/1920 |
