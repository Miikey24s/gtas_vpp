...
| 2026-07-26 | Popover note editor in Order Create | Mỗi dòng mặt hàng chỉ có icon ghi chú đặt ngay bên trái nút xóa; ghi chú cấp đơn rời khỏi thân panel và dùng nút đầy đủ `Ghi chú đơn` bên trái action chính. Cả hai mở popover theo motif ô lọc/Lịch sử đã duyệt | Owner muốn tăng chiều cao hữu dụng của danh sách mặt hàng, bỏ text ghi chú lặp trên từng dòng và rút gọn action chính thành `Tiếp tục` | M2.09 Order Create + shared order-line editor | Use a two-icon row action slot `note → remove`; keep item/order note editors in one reusable anchored popover; footer order is `Ghi chú đơn → Tiếp tục`; preserve keyboard labels and separate data scopes | M2.09 Atlas now; map to accessible Blazor popover after approval | Implemented — OWNER_REVIEW |
