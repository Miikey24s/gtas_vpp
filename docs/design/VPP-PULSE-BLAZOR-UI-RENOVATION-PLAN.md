...
|---|---|---|---|---|---|---|---|
| 2026-08-13 | Các kỳ đặt hàng — action cố định theo capability | Action chính của mọi dòng là `Xem chi tiết` và deep-link tới workspace Chốt kỳ đúng tháng/năm; bỏ dialog chi tiết kỳ cũ. Menu `...` luôn hiện cùng thứ tự `Chốt kỳ → Gia hạn kỳ → Sửa lịch`; action chưa đủ điều kiện vẫn hiện disabled thay vì biến mất | Owner không muốn action chính đổi nghĩa theo trạng thái và không thể biết một action bị thiếu hay không được phép. Kỳ đang mở có đơn không được sửa lịch theo guard backend, nhưng UI cần biểu đạt rule này bằng disabled action ổn định | Local `ADMIN-ROW-ACTIONS`, dùng shared menu capability | Cố định primary detail navigation; projection menu đủ ba action với `Disabled`; cho phép mở menu ngay cả khi cả ba action đều disabled; xóa `Dialog_OrderPeriodDetails` và test luồng cũ | `OrderPeriodManagementWorkspace`, shared `VppAdminActionMenu`, period route-real tests | IMPLEMENTED — QA PASS; architecture `55/55`, route-real responsive/menu `4/4`, deep-link `1/1`, build output cô lập 0 warning/error, visual 1366 reviewed |
