...
- Owner refinement 2026-08-15 (`FILTER-COVERAGE`): màn quản trị chỉ thêm facet có giá trị tra cứu rõ, không sao chép máy móc mọi cột thành filter. Thứ tự canonical đã khóa theo cột hiển thị: Loại danh mục `Trạng thái`; Mặt hàng `Danh mục → Đơn vị → Nhà cung cấp → Trạng thái`; Nhà cung cấp `Trạng thái`; Phòng ban `Phòng ban cha → Trạng thái`; Danh sách bảng giá `Nhà cung cấp → Trạng thái`; Giá mặt hàng `Danh mục → Đơn vị → Trạng thái giá`. Filter chạy trên full authorized dataset trước paging, clear-filter reset toàn bộ facet và toolbar phải wrap trong surface ở độ rộng laptop.
