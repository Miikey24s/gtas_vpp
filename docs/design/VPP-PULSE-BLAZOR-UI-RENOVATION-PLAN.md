...
| 2026-07-24 | M2.09 source-aligned order-create flow | Không lặp title/helper trong content; toolbar chỉ giữ trở về đơn hàng và đúng hai bước `Chọn mặt hàng → Kiểm tra và gửi` | Owner hỏi số bước thật sau khi mockup tự tạo ba bước và hiển thị heading thừa | Order Create mockup + source authority | Source `Page_OrderCreate` defines `StepCount => 2`; submission is the primary action in step 2, not a third step. Remove draft heading/action invented by the mockup | M2.09 create/edit/additional order flow | Implemented — OWNER_REVIEW |
...
| 2026-07-24 | Catalog read-only simplification | Danh mục mặt hàng chỉ là collection tra cứu full-width; không có inspector cố định và không có `Thêm vào đơn` | Owner xác nhận Catalog không tham gia luồng tạo đơn và yêu cầu giữ motif chung nhưng đơn giản hóa | M2 Product Catalog | Compose from collection workspace; retain search/category/unit filter and optional download only; remove order mutation affordance and persistent detail panel | M2.11 Product Catalog | Implemented — OWNER_REVIEW |
...
| 2026-07-24 | Data navigation mode by task intent | Order composition and operational review queues use internal scroll + virtualization; view/lookup and record-management lists use server paging | Owner chốt màn tương tác nhiều phải giữ mạch thao tác liên tục, còn màn chủ yếu xem dùng paging | Shared DataGrid primitive + all Atlas data screens | Use `scroll-virtualized` for My Orders items, Order Create, approval queues and detail item regions; retain paging in Catalog/History list/management/library/users/reports; add Atlas contract assertions | M2–M7 data surfaces | Implemented — OWNER_REVIEW |
...
| 2026-07-24 | Atlas-wide pager/virtualization audit | Mỗi screen có data navigation mode xác định và được regression kiểm tra trên cả ba viewport; action-heavy queues không có pager vì dùng virtualization | Owner yêu cầu phân biệt rõ màn thao tác liên tục với màn tra cứu/xem dữ liệu | 28-screen Design Atlas renderer | Lock paging consumers (History list, Catalog, Management, Library, Users, Reports) and virtualized consumers (My Orders, Order Create, approval queues, History/Management/Approval detail); finite permission matrix remains unpaged | M0–M8 Atlas responsive suite | Verified — OWNER_REVIEW |
...
| 2026-07-26 | Compact note surfaces in Order Create | Ghi chú từng mặt hàng dùng dải nền nhạt có biểu tượng và chỉ chuyển sang trạng thái nhập khi tương tác; ghi chú chung của đơn dùng ô nền nhạt riêng, không lặp underline theo từng dòng | Owner nhận thấy chuỗi underline liên tiếp trong panel hẹp gây rối thị giác và làm ghi chú giống bảng biểu hơn nội dung hỗ trợ | M2.09 Order Create + shared order-line editor | Replace repeated label/underline pairs with one-line note summaries, visible edit affordance on hover/focus and a distinct filled order-level note surface; preserve per-item and order-level data scopes | M2.09 Atlas now; map to accessible Blazor edit control after approval | Implemented — OWNER_REVIEW |
...
| 2026-07-24 | Data-surface continuation and unified footer | Vùng trống của paged table để sạch, không dùng row guide/fake row; dòng dữ liệu cuối luôn có border dưới; virtualized list bỏ spacer rỗng và dùng footer cùng motif pager nhưng chỉ hiển thị tổng/ngữ cảnh, không có số trang | Owner thấy row guides và khoảng thở cuối scroll thiếu đồng bộ, muốn cả hai mode có điểm kết thúc rõ ràng | Shared paged-grid and virtualized-grid primitives | Keep the flexible blank surface between the last row and pager; preserve the final row separator; add a fixed summary-only footer after every virtualized region (for example `18 mặt hàng trong đơn` or `286 mặt hàng khả dụng · Cuộn để xem thêm`); never render page-number controls for virtualization | All paging and virtualized Atlas screens | Implemented — OWNER_REVIEW |
