...
| 2026-07-22 | Shared directional navigation indicators | Active line là một indicator dùng chung: tab trượt trái/phải, sidebar trượt lên/xuống; không còn line bung từ tâm item mới | Owner yêu cầu chuyển động selection thể hiện quan hệ vị trí giữa item cũ và mới như Apple Music, đồng thời muốn underline primary tabs đều nhau nhưng tuyệt đối không dài hơn chữ | Shared RadzenTabs + RadzenPanelMenu interaction runtime | Sidebar dùng vertical FLIP/stretch-settle indicator 180ms, tự theo active route/depth/collapse; primary tab dùng common width `min(60px, shortest label box)` và căn giữa theo DOM label thật | Mọi authenticated shell, primary header-tabs và sidebar expand/collapse | Implemented — OWNER_REVIEW |
