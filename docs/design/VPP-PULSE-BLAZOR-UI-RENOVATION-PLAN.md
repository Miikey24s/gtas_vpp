...
|---|---|---|---|---|---|---|---|
| 2026-08-13 | Các kỳ đặt hàng + grid compact | Mỗi dòng luôn có hai thao tác chính trực tiếp: `Xem` và `Chốt kỳ`; `Chốt kỳ` hiển thị disabled khi chưa đủ điều kiện. Menu `...` chỉ giữ `Gia hạn kỳ → Sửa lịch → Xóa kỳ`; bỏ `Mở lại nhận đơn` khỏi collection. DataGrid có ít bản ghi vẫn giữ chiều cao dòng compact, không kéo giãn hàng để lấp đầy thân grid; empty state vẫn lấp đầy và căn giữa | Owner muốn thao tác quan trọng nhìn thấy ngay, câu chữ ngắn hơn và không còn capability mở lại nhận đơn. Grid Chốt kỳ chỉ có bốn dòng nhưng bị flex kéo cao gây sai mật độ của design system | Global `DATA-ROW` + local `ADMIN-ROW-ACTIONS`, `CAPABILITY-SURFACE` | Đưa Xem/Chốt kỳ ra action trực tiếp theo thứ tự cố định; giữ capability phụ trong overflow và disabled theo nghiệp vụ; ngừng publish reopen tại route. Đổi flex table populated về content-sized, giữ override full-height riêng cho empty/filtered-empty | Mọi DataGrid canonical có ít dữ liệu; `OrderPeriodManagementWorkspace`, period route-real tests | IMPLEMENTED — QA PASS; frontend build 0 warning/error; architecture `12/12`; route-real responsive `8/8` tại 390/768/1366/1920; visual 1920 reviewed |
