...
const adminFormDefinitions = {
  ...
  "library-price-lists": { label:"bảng giá", general:[["Mã bảng giá","PL-2026-H2"],["Tên bảng giá","Bảng giá nửa cuối 2026"],["Ngày hiệu lực","01/07/2026"],["Ngày hết hiệu lực","31/12/2026"],["Trạng thái","Đã công bố"]], relations:[["Nhà cung cấp","An Phát Office"]] },
  "library-prices": { label:"giá mặt hàng", general:[["Đơn giá","145.000 ₫"],["Thuế VAT","8%"],["MOQ","1"],["Thời gian giao","2 ngày"],["Mã hàng NCC","AP-A4-70"]], relations:[["Bảng giá","PL-2026-H2"],["Mặt hàng","Giấy in A4 Double A 70gsm"],["Đơn vị","Ram"]] },
...
function chart(title = "Xu hướng số lượng theo kỳ", options = {}) {
  ...
  const legend = options.singleSeries
    ? badge(options.primaryLegend || "Tổng giá trị", "blue")
    : `${badge(options.primaryLegend || "Đơn thường", "blue")} ${badge(options.secondaryLegend || "Bổ sung", "green")}`;
  return `<section class="card chart-card"><div class="card-header"><div><h3>${title}</h3><p>${options.subtitle || "Đơn thường và đơn bổ sung · 6 kỳ gần nhất"}</p></div><div>${legend}</div></div><div class="chart-area">${bars.map((pair, index) => `<div class="bar-group"><i class="bar" style="height:${pair[0]}px"></i>${options.singleSeries ? "" : `<i class="bar alt" style="height:${pair[1]}px"></i>`}<span class="bar-label">${labels[index]}</span></div>`).join("")}</div></section>`;
}
...
function myOrders(screen) {
  ...
  return shell(screen, `<div class="page-stack">
    ...
    <div class="segment-grid" role="radiogroup" aria-label="Chọn nhóm đơn"><article role="radio" tabindex="0" aria-checked="true" class="segment-card active"><strong>Đơn kỳ hiện tại</strong><span>18 mặt hàng · tổng số lượng 86 · Đã gửi</span></article><article role="radio" tabindex="0" aria-checked="false" class="segment-card"><strong>Đơn bổ sung</strong><span>Chưa có đơn · Có thể tạo trong kỳ mở</span></article><article role="radio" tabindex="0" aria-checked="false" class="segment-card"><strong>Kỳ trước · 06/2026</strong><span>12 mặt hàng · Chỉ xem</span></article></div>
    <section class="card data-card order-detail-panel is-wide"><div class="card-header order-detail-header"><div><h3 data-short-order-code title="DEMO-PPJ-0726-REV-0001">DEMO-PPJ-0726</h3><button type="button" data-copy-order-code hidden style="display:none">Sao chép mã đầy đủ</button><p>Gửi lúc 09:42, 23/07/2026 · ${badge("Đã gửi", "blue")}</p></div><div class="page-actions">${button("Sửa", "quiet", "edit")}${linkButton("history", "Lịch sử đơn", "quiet", "history")}${orderExportActions(true)}${button("Hủy", "danger", "cancel")}</div></div>${orderItemsRegion(rows)}</section>
    <section class="card state-card" data-my-orders-empty-variant hidden style="display:none"><div><span class="state-icon">${icon("shopping_bag")}</span><h3>Chưa có đơn trong kỳ này</h3><p>Tạo đơn mới hoặc sao chép đơn kỳ trước để bắt đầu nhanh hơn.</p><div class="page-actions">${button("Tạo đơn kỳ này","primary","add")}${button("Sao chép kỳ trước","quiet","content_copy")}</div></div></section>
  </div>`);
}
...
function history(screen) {
  const rows = [
    ...
    ["3", "05/2026", "DEMO-0526", badge("Đơn thường", "blue"), badge("Đã gửi", "blue"), "11", "47", "19/05/2026", "–"],
    ["4", "04/2026", "DEMO-0426", badge("Đơn thường", "blue"), badge("Đã gửi", "blue"), "8", "31", "17/04/2026", "–"],
...
function management(screen, all = false) {
  if (!all) {
    const rows = [
      ...
      ["3", "07/2026", "DEMO-MINH-0726", "Phạm Tuấn Minh", badge("Đơn thường", "blue"), badge("Đã gửi", "blue"), "22/07/2026", "Vật tư dự án"],
      ["4", "07/2026", "DEMO-HUNG-0726", "Lê Việt Hùng", badge("Đơn thường", "blue"), badge("Đã gửi", "blue"), "21/07/2026", "Thiết bị nhóm"],
      ["5", "07/2026", "DEMO-LAN-0726", "Nguyễn Thu Lan", badge("Đơn thường", "blue"), badge("Đã gửi", "blue"), "20/07/2026", "–"]
    ];
    const detailRows = expandIndexedRows([
    ...
    const summaryKpis = kpis([
      ...
      {label:"Dòng mặt hàng",value:"42",icon:"format_list_numbered",meta:"Theo các đơn đang hiển thị"},
  ...
  const rows = [
    ...
    ["4", "07/2026", "DEMO-OPS-0726", "Phạm Hoàng Sơn", "Vận hành", badge("Đơn thường", "blue"), badge("Đã gửi", "blue"), "21/07/2026"],
...
function approval(screen) {
  const queueSeedRows = [
    ...
    ["SUP-0726-006", "Phạm Minh Anh", "OPS", "5", "27", "Mở rộng ca trực", badge("Chờ duyệt", "amber")]
  ];
  ...
  return shell(screen, masterDetailWorkspace({
    ...
    detail: approvalOrderDetail({
      ...
      fields: [["Kỳ","07/2026"],["Loại đơn",badge("Đơn bổ sung","amber")],["Người đặt","Nguyễn Thanh Thủy"],["Phòng ban","IT"]],
      rows: detailRows,
      quantity: "48",
      note: "Thiếu vật tư dự án",
      supportingMeta: "Đã chờ 4 giờ 12 phút",
...
function periodReview(screen) {
  const blockerRows = [
    ...
    ["SUP-0726-006","Phạm Minh Anh","OPS","Mở rộng ca trực",badge("Chờ quản lý xử lý","amber")]
  ];
  return shell(screen, `<div class="page-stack period-operation-workspace">${periodFlowNav("review")}<section class="period-hero is-blocked"><div><div class="period-eyebrow">Kỳ 07/2026 · Còn nhận đơn</div><div class="period-title">Chưa thể chốt kỳ</div><div class="period-meta">Còn 2 đơn bổ sung cần quản lý xử lý</div></div><div class="period-hero-actions">${linkButton("supplement-approval", "Duyệt đơn bổ sung", "", "fact_check")}${button("Chốt kỳ", "disabled", "lock")}</div></section><div class="split-layout period-review-layout"><section class="card"><div class="card-header"><div><h3>Điều kiện trước khi gom đơn</h3><p>Quản lý rà soát điều kiện có nguồn từ bản xem trước chốt kỳ</p></div></div><div class="readiness-list"><div class="readiness-row">${icon("check_circle")}<div><p>Phiên bản đơn hiện hành hợp lệ</p><small>86 đơn đã được kiểm tra theo phiên bản mới nhất</small></div>${badge("Đạt", "green")}</div><div class="readiness-row">${icon("error")}<div><p>Đơn bổ sung chưa được xử lý</p><small>Quản lý có quyền duyệt hoặc từ chối đơn bổ sung</small></div>${badge("Chặn", "red")}</div><div class="readiness-row">${icon("schedule")}<div><p>Chờ hết hạn nhận đơn</p><small>Chưa thể lưu kết quả khi dữ liệu vẫn có thể thay đổi</small></div>${badge("14 ngày", "blue")}</div></div></section>${tableCard("Điều kiện ngăn chốt kỳ đang theo dõi","Mở màn hình duyệt đơn bổ sung để xử lý",["Mã đơn","Người đặt","Phòng","Ghi chú","Trạng thái"],blockerRows,{className:"period-blocker-list",summary:"Hiển thị 1–2 trên 2 điều kiện",pageCount:1})}</div></div>`);
}

function periodDemand(screen) {
  const orderRows = [
    ["DEMO-PPJ-0726","Nguyễn An Nam","IT","18","86",badge("Đã gửi","blue")],
    ["SUP-HR-0726","Nguyễn Bảo Trâm","HR","3","12",badge("Đã duyệt","green")],
    ["DEMO-FA-0726","Lê Minh Châu","FA","15","96",badge("Đã gửi","blue")],
    ["DEMO-OPS-0726","Phạm Hoàng Sơn","OPS","24","188",badge("Đã gửi","blue")],
    ["DEMO-MKT-0726","Vũ Thanh Hà","MKT","10","63",badge("Đã gửi","blue")]
  ];
  const demandRows = expandIndexedRows([
    ["1",`<strong>Giấy in A4 Double A 70gsm</strong><span class="subline">VPP_7B9F34EEC2C489</span>`,"Ram","146"],
    ["2",`<strong>Bút bi Thiên Long TL-027</strong><span class="subline">VPP_74CBE20A69F8</span>`,"Cây","284"],
    ["3",`<strong>Pin 2A Energizer</strong><span class="subline">VPP_B5DF2B25582F</span>`,"Vỉ","42"]
  ],18,"DEMAND",{ name: 1, unit: 2 }).map((row, index) => {
    if (index < 3) return row;
    row[3] = String(12 + ((index * 11) % 220));
    return row;
  });
  return shell(screen, `<div class="page-stack period-operation-workspace">${periodFlowNav("demand")}${filterBar({search:"Tìm mã đơn hoặc mặt hàng",filters:["Kỳ 07/2026","Đơn hợp lệ hiện hành"],action:linkButton("supply-allocation","Tiếp tục chọn nguồn cung","primary","arrow_forward")})}${kpis([{label:"Đơn hợp lệ",value:"86",icon:"receipt_long",meta:"Phiên bản hiện hành"},{label:"Mặt hàng duy nhất",value:"286",icon:"inventory_2",meta:"Đã gộp không trùng"},{label:"Tổng số lượng",value:"4.820",icon:"stacked_bar_chart",meta:"Đơn thường + bổ sung đã duyệt"},{label:"Điều kiện ngăn chốt",value:"0",icon:"task_alt",meta:"Theo bản xem trước hiện hành"}])}<div class="split-layout period-demand-layout">${tableCard("Theo đơn","Chỉ phiên bản hợp lệ được đưa vào dữ liệu tổng hợp kỳ 07/2026",["Mã đơn","Người đặt","Phòng",{label:"Dòng mặt hàng",align:"number"},{label:"Số lượng",align:"number"},"Trạng thái"],orderRows,{summary:"Hiển thị 1–5 trên 86 đơn",pageCount:3})}${virtualizedTableCard("Theo mặt hàng","Gom từ phiên bản hợp lệ mới nhất",["#","Mặt hàng","Đơn vị",{label:"Số lượng",align:"number"}],demandRows,{className:"period-demand-items",summary:"Tổng cộng 286 mặt hàng"})}</div></div>`);
}
...
function settlement(screen) {
  ...
  return shell(screen, `<div class="page-stack period-operation-workspace settlement-workspace">${periodFlowNav("settle")}${kpis([{label:"Mặt hàng",value:"286",icon:"inventory_2",meta:"Đủ 100% nguồn giá"},{label:"Tổng số lượng",value:"4.820",icon:"stacked_bar_chart",meta:"Từ 86 đơn hợp lệ"},{label:"Ngoại lệ",value:"2",icon:"call_split",meta:"Đều có lý do và nhật ký"},{label:"Tạm tính",value:"79,5M",icon:"payments",meta:"Đã gồm VAT theo bảng giá"}])}<div class="split-layout settlement-layout">${tableCard("Phân bổ nguồn cung","Xem trước dữ liệu sẽ được lưu tại thời điểm chốt kỳ",["Nhà cung cấp","Bảng giá",{label:"Mặt hàng",align:"number"},{label:"Số lượng",align:"number"},{label:"Tạm tính",align:"number"}],settlementRows,{summary:"Hiển thị 1–3 trên 3 nguồn cung",pageCount:1})}<section class="card settlement-confirmation"><div class="card-header"><div><h3>Xác nhận chốt kỳ 07/2026</h3><p>Lưu kết quả chốt kỳ và khóa phiên bản đơn hiện hành</p></div>${badge("Sẵn sàng","green")}</div><div class="card-body"><div class="account-alert">${icon("lock")}<span>Sau khi xác nhận, đơn trong kỳ chuyển sang chỉ xem. Nguồn cung, bảng giá, ngoại lệ và tổng tiền đã lưu không tự thay đổi theo dữ liệu danh mục sau này.</span></div><div class="readiness-list compact"><div class="readiness-row">${icon("check_circle")}<div><p>Đơn hợp lệ đã được gom</p><small>86 phiên bản hiện hành</small></div>${badge("Đạt","green")}</div><div class="readiness-row">${icon("check_circle")}<div><p>Độ phủ giá đạt 100%</p><small>Nhà cung cấp chính + 2 ngoại lệ</small></div>${badge("Đạt","green")}</div><div class="readiness-row">${icon("check_circle")}<div><p>Nhật ký ngoại lệ đầy đủ</p><small>Có lý do, người chọn và bảng giá nguồn</small></div>${badge("Đạt","green")}</div></div><div class="settlement-revision-variant" data-correction-variant><h4>Biến thể hiệu chỉnh</h4><p>Khi kỳ đã chốt, hệ thống yêu cầu lý do 5–500 ký tự và người hiệu chỉnh khác người chốt phiên bản trước.</p><div class="form-group settlement-reason"><label>Lý do hiệu chỉnh</label><div class="input">Điều chỉnh nguồn cung theo biên bản đối soát</div></div></div></div><div class="inspector-actions">${linkButton("supply-allocation","Quay lại nguồn cung","quiet","arrow_back")}${button("Xác nhận và chốt kỳ","primary","lock")}</div></section></div></div>`);
}

const libraryDefinitions = {
  "library-classes": { search:"Tìm mã hoặc tên loại", title:"Loại danh mục", subtitle:"Định nghĩa nhóm giá trị dùng chung trong hệ thống", total:1, noun:"loại danh mục", headers:["Mã loại","Tên loại","Mô-đun","Trạng thái"], rows:[["Uom","Đơn vị tính","VPP",badge("Hoạt động","green")]]},
  "library-categories": { search:"Tìm mã hoặc tên danh mục", title:"Danh mục", subtitle:"Nhóm phân loại trực tiếp của mặt hàng", total:18, noun:"danh mục", headers:["Mã","Tên danh mục","Trạng thái"], rows:[["PAPER","Giấy in các loại",badge("Hoạt động","green")],["PEN","Bút viết",badge("Hoạt động","green")],["FILE","Bìa và hồ sơ",badge("Hoạt động","green")],["TAPE","Băng keo, bấm kim",badge("Hoạt động","green")],["BATTERY","Pin và thiết bị nhỏ",badge("Ngừng áp dụng","red")]]},
  "library-items": { search:"Tìm mã hoặc tên mặt hàng", title:"Mặt hàng", subtitle:"Dữ liệu mặt hàng dùng cho danh mục và đơn đặt", total:286, noun:"mặt hàng", headers:["Mã mặt hàng","Tên mặt hàng","Đơn vị","Danh mục","Trạng thái"], rows:[["VPP_7B9F34EE","Giấy in A4 Double A 70gsm","Ram","Giấy in",badge("Hoạt động","green")],["VPP_74CBE20A","Bút bi Thiên Long TL-027","Cây","Bút viết",badge("Hoạt động","green")],["VPP_D942B0AF","File còng 7cm Kokuyo","Cái","Bìa hồ sơ",badge("Hoạt động","green")],["VPP_B5DF2B25","Pin 2A Energizer","Vỉ","Thiết bị",badge("Ngừng áp dụng","red")],["VPP_16B403E8","Băng keo trong 5cm 100y","Cuộn","Băng keo",badge("Hoạt động","green")]]},
  "library-suppliers": { search:"Tìm tên viết tắt hoặc nhà cung cấp", title:"Nhà cung cấp", subtitle:"Đối tác cung ứng và thông tin địa chỉ", total:5, noun:"nhà cung cấp", headers:["Tên viết tắt","Tên nhà cung cấp","Địa chỉ","Tỉnh/Thành phố","Trạng thái"], rows:[["ANPHAT","An Phát Office","12 Nguyễn Văn Linh","TP. Hồ Chí Minh",badge("Hoạt động","green")],["MINHHAI","Minh Hải Stationery","28 Trần Duy Hưng","Hà Nội",badge("Hoạt động","green")],["HONGPHUC","Hồng Phúc Trading","16 Nguyễn Văn Thoại","Đà Nẵng",badge("Hoạt động","green")],["VIETPHUONG","Việt Phương","41 Lý Tự Trọng","TP. Hồ Chí Minh",badge("Hoạt động","green")],["VP24H","Văn phòng 24h","87 Cầu Giấy","Hà Nội",badge("Ngừng áp dụng","red")]]},
  ...
  "library-prices": { search:"Tìm mặt hàng trong bảng giá", title:"Giá mặt hàng", subtitle:"PL-2026-H2 · An Phát Office", total:286, noun:"mức giá", headers:["Mã mặt hàng","Tên mặt hàng","Đơn vị",{label:"Đơn giá",align:"number"},"Thuế","MOQ"], rows:[["VPP_7B9F34EE","Giấy in A4 Double A 70gsm","Ram","145.000 ₫","8%","1"],["VPP_74CBE20A","Bút bi Thiên Long TL-027","Cây","4.500 ₫","8%","10"],["VPP_D942B0AF","File còng 7cm Kokuyo","Cái","68.000 ₫","8%","1"],["VPP_B5DF2B25","Pin 2A Energizer","Vỉ","42.000 ₫","10%","2"],["VPP_16B403E8","Băng keo trong 5cm 100y","Cuộn","18.000 ₫","8%","1"]]},
  "library-departments": { search:"Tìm mã hoặc tên phòng ban", title:"Phòng ban", subtitle:"Phạm vi phòng ban dùng cho đơn yêu cầu và báo cáo", total:9, noun:"phòng ban", headers:["Mã","Tên phòng ban","Phòng ban cấp trên","Trạng thái"], rows:[["IT","Công nghệ thông tin","—",badge("Hoạt động","green")],["HR","Nhân sự","—",badge("Hoạt động","green")],["FA","Tài chính – Kế toán","—",badge("Hoạt động","green")],["OPS","Vận hành","—",badge("Hoạt động","green")],["MKT","Marketing","—",badge("Hoạt động","green")]]}
};
...
function library(screen) {
  ...
  const terminalAction = screen.template === "library-price-lists"
    ...
    : softDeleteButton();
  return shell(screen, masterDetailWorkspace({
    filters: filterBar({search:data.search,filters,action:`${columnPresetControl()}${button(`Thêm ${data.title.toLowerCase()}`,"primary","add")}`}),
    ...
    detail: adminInspector(String(selected[1]), String(selected[0]), [["Trạng thái",detailStatus],["Ngày tạo","01/07/2026"],["Cập nhật","23/07/2026"]], `${button("Chỉnh sửa", "", "edit")}${relatedAction || button("Xem lịch sử", "quiet", "history")}${terminalAction}`),
...
function permissions(screen) {
  ...
  return shell(screen, `<div class="page-stack permissions-page"><section class="card permissions-card"><div class="card-header"><div><h3>Ma trận quyền thao tác</h3><p>Ba vai trò thống nhất: Nhân viên, Quản lý và Quản trị hệ thống</p></div><div class="page-actions">${linkButton("users", "Quản lý người dùng", "quiet", "group")}${button("Xem nhật ký","", "history")}</div></div><div class="permission-matrix-scroll virtualized-grid" data-grid-mode="scroll-virtualized"><table class="permission-matrix"><thead><tr><th>Quyền thao tác</th>${groups.map(group=>`<th class="${group === "DEV" ? "developer-owner-column" : ""}">${group}<span class="subline">${roleLabels[group]}</span></th>`).join("")}</tr></thead><tbody>${actions.map(([code, name, grants])=>`<tr><td><strong>${name}</strong><span class="subline">${code}</span></td>${grants.map((value,index)=>`<td class="${groups[index] === "DEV" ? "developer-owner-column" : ""}">${value?`<span class="permission-check">${icon("check")}</span>`:`<span class="permission-none">—</span>`}</td>`).join("")}</tr>`).join("")}</tbody></table></div>${virtualizedFooter("Tổng cộng 18 quyền thao tác · 3 vai trò cố định")}</section><div class="two-column permission-notes"><section class="card"><div class="card-header"><div><h3>DEV · Quản trị hệ thống</h3><p>Có toàn bộ quyền giao diện để quản trị và kiểm thử hệ thống</p></div>${badge("Toàn quyền","blue")}</div><div class="card-body"><div class="account-alert">${icon("security")}<span>Quyền truy cập đầy đủ vẫn phải tuân theo các điều kiện nghiệp vụ, thời hạn, giới hạn và dữ liệu đã chốt.</span></div><p class="permission-note-copy">Ma trận quyền thao tác là hợp đồng chuẩn chỉ đọc; thay đổi phân công người dùng được thực hiện ở màn quản lý người dùng.</p></div></section><section class="card"><div class="card-header"><div><h3>Phạm vi ba vai trò</h3><p>Quyền được trình bày theo ngôn ngữ nghiệp vụ</p></div>${badge("Đang áp dụng","green")}</div><div class="card-body"><dl class="key-values"><dt>Nhân viên</dt><dd>Tạo và theo dõi đơn của bản thân</dd><dt>Quản lý</dt><dd>Duyệt đơn bổ sung, quản lý danh mục, báo cáo và chốt kỳ</dd><dt>Quản trị hệ thống</dt><dd>Toàn bộ quyền và quản lý người dùng</dd><dt>Nguồn chuẩn</dt><dd>CanonicalRbac · 18 quyền thao tác</dd></dl></div></section></div></div>`);
}

function reports(screen) {
  const rows = [["IT","18","122","18.420.000 ₫"],["HR","12","87","10.760.000 ₫"],["FA","15","96","14.250.000 ₫"],["OPS","24","188","26.870.000 ₫"],["MKT","10","63","9.340.000 ₫"]];
  return shell(screen, `<div class="page-stack">${filterBar({search:"Tìm mã phòng ban",filters:["Kỳ 07/2026","Toàn công ty","Phiên bản hiện hành"],action:`${linkButton("period-demand", "Xem nguồn dữ liệu kỳ", "quiet", "table_view")}${button("Xóa bộ lọc","quiet","filter_alt_off")}${button("Xuất CSV","","description")}${button("Xuất Excel","primary","table_view")}`})}${kpis([{label:"Tổng đơn",value:"86",icon:"receipt_long",meta:"Theo phạm vi đang chọn"},{label:"Dòng mặt hàng",value:"640",icon:"format_list_numbered",meta:"Từ các đơn hiện hành"},{label:"Tổng số lượng",value:"4.820",icon:"inventory_2",meta:"Đơn vị yêu cầu"},{label:"Tổng giá trị",value:"79,6M",icon:"payments",meta:"Theo dữ liệu báo cáo"},{label:"Người đặt",value:"89",icon:"group",meta:"Tài khoản có đơn"}])}<div class="split-layout">${chart("Tổng giá trị theo kỳ",{singleSeries:true,primaryLegend:"Tổng giá trị",subtitle:"Dữ liệu PeriodTrend.TotalAmount theo phạm vi báo cáo"})}${tableCard("Theo mã phòng ban","Số liệu từ DTO báo cáo hiện hành",["Mã",{label:"Số đơn",align:"number"},{label:"Số lượng",align:"number"},{label:"Tổng giá trị",align:"number"}],rows,{summary:"Hiển thị 1–5 trên 9 mã phòng ban",pageCount:2})}</div></div>`);
}

function libraryClasses(screen) {
  const classRows = [
    ["Uom", "Đơn vị tính", "VPP", badge("Hoạt động", "green")],
    ["ItemSource", "Nguồn mặt hàng", "VPP", badge("Hoạt động", "green")]
  ];
  const valueRows = [
    ["RAM", "Ram", "10", "Đơn vị giấy in", badge("Hoạt động", "green")],
    ["CAI", "Cái", "20", "Đơn vị phổ biến", badge("Hoạt động", "green")],
    ["CAY", "Cây", "30", "Đơn vị bút viết", badge("Hoạt động", "green")],
    ["CUON", "Cuộn", "40", "Đơn vị băng keo", badge("Hoạt động", "green")]
  ];
  return shell(screen, `<div class="page-stack library-class-workspace">${filterBar({search:"Tìm mã hoặc tên loại",filters:["Tất cả trạng thái"],action:drawerButton("Thêm loại danh mục","primary","add","create")})}<div class="split-layout">${tableCard("Loại danh mục","Chọn một loại để xem và quản lý các giá trị",["Mã loại","Tên loại","Mô-đun","Trạng thái"],classRows,{className:"admin-entity-list",summary:"Hiển thị 1–2 trên 2 loại",pageCount:1})}${tableCard("Giá trị · Đơn vị tính","Các mã giá trị thuộc loại đang chọn",["Mã","Giá trị","Thứ tự","Mô tả","Trạng thái"],valueRows,{className:"admin-entity-list",headerAction:drawerButton("Thêm giá trị","primary","add","create"),summary:"Hiển thị 1–4 trên 12 giá trị",pageCount:3})}</div></div>`);
}
...
function accountScreen(screen) {
  const configs = {
    ...
    "account-reset": {title:"Đặt lại mật khẩu",desc:"Tạo mật khẩu mới cho tài khoản của bạn.",fields:[["Mật khẩu mới","••••••••••"],["Xác nhận mật khẩu mới","••••••••••"]],action:"Đặt lại mật khẩu",actionTarget:"login",alert:"Mật khẩu phải có ít nhất 10 ký tự, gồm chữ hoa, chữ thường, số và ký tự đặc biệt.",links:[["Quay lại đăng nhập","login"]]},
    "account-change": {title:"Đổi mật khẩu",desc:"Xác nhận mật khẩu hiện tại trước khi tạo mật khẩu mới.",fields:[["Mật khẩu hiện tại","••••••••••"],["Mật khẩu mới","••••••••••"],["Xác nhận mật khẩu mới","••••••••••"]],action:"Đổi mật khẩu",actionTarget:"my-orders",alert:"Mật khẩu mới phải có ít nhất 10 ký tự, gồm chữ hoa, chữ thường, số và ký tự đặc biệt.",links:[["Quay lại ứng dụng","my-orders"]]},
    ...
    "account-register": {title:"Tạo tài khoản",desc:"Tạo tài khoản để sử dụng trong môi trường phát triển và kiểm thử.",fields:[["Tên đăng nhập","annam"],["Họ và tên","Nguyễn An Nam"],["Email công ty","annam@gtas.local"],["Mật khẩu","••••••••••"],["Xác nhận mật khẩu","••••••••••"]],action:"Tạo tài khoản",actionTarget:"login",alert:"Nếu thông tin hợp lệ, yêu cầu đăng ký được tiếp nhận và chờ quản trị viên phê duyệt.",links:[["Quay lại đăng nhập","login"]]}
  };
...
function renderTemplate(screen) {
  ...
  if (screen.template === "library-classes") return libraryClasses(screen);
