...
| E14 | Lập plan nhiều wave khi owner có pool Plus nhưng quota thay đổi theo thời điểm | Tra tài liệu OpenAI chính thức hiện tại; hỏi capacity một lần nếu task chưa có; ghi snapshot kèm ngày; đặt `Model + effort` trong bảng canonical; chỉ đổi ở checkpoint lớn; phân biệt khuyến nghị với model active | Hardcode account count thành luật lâu dài, đổi model từng file, dùng flagship cho mọi bước hoặc khai đã tự switch khi surface không hỗ trợ |
