...
| E14 | Lập plan nhiều wave khi owner có pool Plus nhưng quota thay đổi theo thời điểm | Tra tài liệu OpenAI chính thức; probe quota ẩn danh trước routing; báo coverage, model/effort + lý do, estimate range `% Plus-equivalent`, confidence/source, buffer và `ENOUGH/SLICE_ONLY/WAIT`; đo checkpoint để reforecast; không hạ chất lượng chỉ để vừa quota | Hỏi lại dù probe dùng được, in key/account detail, báo % chính xác khi chưa có history, coi weekly-only là đủ full plan, hardcode count, thêm poller trùng, tự downgrade, đổi model từng file hoặc khai đã switch |
