...
- Owner-review correction T001: pane `Đơn đang tạo` dùng cùng grid track cho header/row, số lượng có input trực tiếp `1–9999` và vẫn giữ `−/+`. Nút `Tiếp tục` chỉ phụ thuộc đã chọn mặt hàng; lý do đơn bổ sung được cảnh báo rõ ở review và vẫn là validation bắt buộc trước submit, không còn bị giấu trong trạng thái disabled.
- Bước `Kiểm tra và gửi` có isolated CSS/full-height data surface riêng, thay giao diện Radzen/native thô bằng header, summary, requirement warning, grid, footer và action bar cùng motif. T001 hoãn full frontend suite; evidence hiện tại gồm frontend Release build `0 warning/error` và focused isolated Playwright `AdditionalOrder_DirectQuantityAndReviewRemainUsable` pass `1/1`, ảnh đã kiểm bằng mắt ở `tmp/t001-order-create-fast/` (ignored).
- Owner duyệt T001 cho My Orders: ba card `Đơn kỳ hiện tại / Đơn bổ sung / Kỳ trước` chuyển thành segmented selector compact cùng motif chọn kỳ. Period/deadline nằm trong một cụm ngắn; title/summary của segment được đưa xuống một status line và CTA tạo/copy chỉ xuất hiện ở segment liên quan, không trộn action tạo dữ liệu vào selector.
