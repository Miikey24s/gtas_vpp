...
- Chốt kỳ hợp nhất thay workflow bốn bước: selector `Kỳ này / Tùy chọn` và `Theo đơn / Theo phòng ban`, supplier decision strip không có tiêu đề/card KPI, server paging `100` cho đơn và snapshot group cho phòng ban. Toolbar giữ search + loại đơn + trạng thái + phòng ban; chọn nhà cung cấp dùng rich dropdown riêng trong decision strip, không giả dạng filter.
...
- **Chốt kỳ implementation:** owner duyệt concept và yêu cầu triển khai; production route đã chuyển sang workspace hợp nhất, bỏ chuỗi `Phương án chốt`, bỏ KPI/readiness card và không có supplier filter/column. Nhà cung cấp/bảng giá đổi bằng anchored dropdown giàu thông tin, chọn dòng hợp lệ là áp dụng ngay. Focused route-real desktop `1366×768`, tablet `1024×768`, mobile `390×844` pass containment; ảnh ignored tại `tmp/period-settlement-unified/` đã được kiểm bằng mắt.
