...
- Profile cột được đối chiếu lại với `UI-ADMIN-ACCESS-001`: name + code dùng cell hai dòng, Supplier gộp city/ward, Category hiện updated time, Pricing dùng mã mặt hàng hệ thống cùng price/VAT/MOQ/lead và cột phụ chuyển vào column picker. CSS filter/search legacy zero-consumer bị xóa; shared grid contract đang dùng được giữ nguyên.
...
- **Layout regression correction — 2026-07-30:** `PeriodSettlementPanel` sở hữu một scoped wrapper `display: contents` để override đúng grid hai cột của `HistoryWorkspaceShell`; `VppOperationWorkspace` có contract typed `FillAvailableSpace` thay cho min-height hardcode. Geometry test khóa selector, decision strip và data surface thành ba hàng full-width; filter desktop cùng một hàng và surface chạm bottom inset. Isolated Playwright pass main `1/1` + responsive `4/4`; screenshot `1366`, `1920` và `390` đã được kiểm bằng mắt trong output ignored.
- **Admin visual correction — 2026-07-30:** Lookup category tách `Tên loại`, `Vô hiệu hóa` và `Thao tác` thành ba cell đúng semantics; switch không còn nằm trong cell tên hoặc sinh thêm giá trị boolean thô khi column picker hiện cột. Form outlined dùng một focus owner ở `RadzenFormField`; textbox/textarea/dropdown/numeric/datepicker lồng bên trong không vẽ outline/halo thứ hai, đồng thời bỏ radius focus legacy từng làm control đổi hình khi nhận focus. Evidence: frontend Release build `0 warning/error`; frontend unit/architecture `203/203`; isolated Lookup layout + dialog desktop/mobile pass `2/2`, layout rerun có screenshot pass `1/1`; ảnh đã được kiểm bằng mắt ở `tmp/lookup-focus-alignment/` (ignored). `verify -Scope frontend` dừng trước frontend gate tại `model-routing-eval` do nhóm AI-harness owner đang sửa ngoài scope (`62 pass / 1 fail`), không được stage hoặc sửa trong correction này.
- **Selector/order/date/Class correction — 2026-07-30:** selector ngang canonical dùng độ rộng của label dài nhất; active background không còn phụ thuộc JS indicator nên không tái diễn chữ trắng trên nền sáng. My Orders hiện full code + status + thời gian ở cụm trái, action History deep-link sang đúng row/detail; History detail hiện full code và ẩn close icon ở desktop persistent layout. `VppPeriodPickerPopover` thay editor tháng rời ở History và Chốt kỳ, dùng cùng transient motion/outside-click/Escape và z-index token canonical. Lookup Class bỏ splitter kéo tay, dùng fixed master-detail; Code/Tên/Trạng thái tách cột và page reload out-of-range quay về trang đầu. Legacy order-code popover JS/CSS zero-consumer đã xóa.
- Evidence correction mới nhất: frontend Release build `0 warning/error`; frontend unit/architecture `202/202`; focused isolated browser `4/4` pass cho My Orders, History deep-link + period picker, Chốt kỳ period picker và Lookup Class fixed split. Completion-audit rerun `6/6` pass, đo trực tiếp hai selector Chốt kỳ có segment bằng nhau và date picker History đóng được bằng outside-click lẫn `Escape`. Mutation lifecycle cô lập bổ sung `1/1` pass: tạo rồi hủy đơn bổ sung tự refresh policy kỳ, CTA `Tạo đơn bổ sung` xuất hiện lại và enabled mà không reload thủ công. Ảnh runtime đã được kiểm bằng mắt trong `tmp/ui-review-current/` (ignored); không điều khiển hoặc restart `dotnet watch` của owner.
