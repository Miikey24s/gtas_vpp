...
- Evidence correction mới nhất: frontend Release build `0 warning/error`; frontend unit/architecture `202/202`; focused isolated browser `4/4` pass cho My Orders, History deep-link + period picker, Chốt kỳ period picker và Lookup Class fixed split. Mutation lifecycle cô lập bổ sung `1/1` pass: tạo rồi hủy đơn bổ sung tự refresh policy kỳ, CTA `Tạo đơn bổ sung` xuất hiện lại và enabled mà không reload thủ công. Ảnh runtime đã được kiểm bằng mắt trong `tmp/ui-review-current/` (ignored); không điều khiển hoặc restart `dotnet watch` của owner.
