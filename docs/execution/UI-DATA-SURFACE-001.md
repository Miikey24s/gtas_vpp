...
- Owner-review fix round 2: guard row chạy đồng bộ ngay trong scroll event thay vì trễ một animation frame; observer tự nối lại nếu Radzen thay scroll viewport. Import JS có version để `dotnet watch`/browser không giữ module cũ, xử lý trường hợp chỉ dòng mã của cell Mặt hàng hai dòng ló lên header.
