...
- Status: `IN PROGRESS — FR0/FR1/FR2/FR3 COMPLETE; FR4 CATALOG CORE COMPLETE`
- Priority: P1
...
| Baseline hiện tại | Release build sạch; frontend unit/architecture `271/271`; 83 UI test được phát hiện. Account/shell và Lookup/Category/Supplier/Department/Item đã có transport owner rõ; `verify -Scope frontend` pass tại checkpoint FR2 | [Evidence](#plan-detail-evidence) |
...
| Việc làm ngay | Catalog core đã hoàn tất. Tiếp theo migrate Pricing, rồi xóa các consumer `GlobalClass` còn lại và chốt FR4 | [Continuation](#plan-detail-continuation) |
...
- Next exact action: tạo/mở rộng Pricing client cho Price List và Item Price; giữ publish/expire/default/
  clone/compare lifecycle, sau đó retire `GlobalClass` và generic transport khỏi Library.
