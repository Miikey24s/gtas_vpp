...
- Status: `IN PROGRESS — FR0/FR1/FR2/FR3 COMPLETE; FR4 LOOKUP + CATEGORY + SUPPLIER + DEPARTMENT COMPLETE`
- Priority: P1
...
| Baseline hiện tại | Release build sạch; frontend unit/architecture `270/270`; 83 UI test được phát hiện. Account/shell và bốn Library surface đầu đã có transport owner rõ; `verify -Scope frontend` pass tại checkpoint FR2 | [Evidence](#plan-detail-evidence) |
...
| Việc làm ngay | Lookup + Category + Supplier + Department đã hoàn tất. Tiếp theo migrate Item, rồi Pricing và retire `GlobalClass` | [Continuation](#plan-detail-continuation) |
...
- Next exact action: migrate Item vào `CatalogApiClient`, giữ typed `VppItemCreateRequest`/
  `VppItemUpdateRequest`, category/UOM lookup và status endpoint; sau đó Pricing.
