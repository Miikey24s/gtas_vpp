...
| F4 — Pattern | `IMPLEMENTED — OWNER REVIEW` | **Sol · XHigh** — checkpoint kiến trúc khó nhất, ảnh hưởng scalability dài hạn. | Khóa bốn token `page outer inset`; tạo sáu pattern typed/slot-based `Account`, `Collection`, `ListDetail`, `SplitEditor`, `Operation`, `Analytics`; migrate hai consumer thật cho mỗi pattern. | **Khung scalable hoàn chỉnh:** agent có bản đồ chọn pattern, page giữ cùng nhịp với header/sidebar, route vẫn sở hữu API/permission/nghiệp vụ. Đây chưa phải toàn bộ màn đã migrate. | Runtime board 6 archetype + 12 route consumer; evidence local ignored. | Không reflection/endpoint string trong pattern; architecture/frontend pass; 12 route geometry pass; 6 regression interaction/scroll pass. |
