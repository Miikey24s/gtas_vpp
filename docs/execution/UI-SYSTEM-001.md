...
| F3 — Composite | `DONE — OWNER REVIEW ROUND 3` | **Sol · High** — phải suy luận behavior chung từ hai consumer thật, rủi ro abstraction sai. | Trích xuất order-detail filter-to-footer typed dùng chung cho My Orders và History; thêm clear-filter primitive dùng chung; route giữ header/action/API/nghiệp vụ riêng. | **Luồng mẫu hoàn chỉnh:** search, popup lọc, clear-filter, code/note popover, 6 cột, virtualization và footer cùng motif; danh sách đơn giữ paging/footer riêng, My Orders chỉ rộng hơn, History dùng header kiểu PDF. | Review board round 2 + screenshot round 3 của danh sách/chi tiết từ runtime isolated; evidence local ignored. | Build/frontend `188/188`; My Orders + History route-real pass, gồm footer luôn hiện ở một trang, responsive, popup, filter-empty, export và không còn revision-history action. |
