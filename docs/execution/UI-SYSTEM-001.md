...
| F3 — Composite | `DONE — OWNER REVIEW ROUND 2` | **Sol · High** — phải suy luận behavior chung từ hai consumer thật, rủi ro abstraction sai. | Trích xuất order-detail filter-to-footer typed dùng chung cho My Orders và History; route giữ header/action/API/nghiệp vụ riêng. | **Luồng mẫu hoàn chỉnh:** search, popup lọc, code/note popover, 6 cột, virtualization và footer giống nhau; My Orders chỉ rộng hơn, History dùng header kiểu PDF. | Review board round 2 My Orders + History từ runtime isolated; evidence local ignored. | Build/frontend `188/188`; My Orders + History route-real pass, gồm responsive, popup, filter-empty, export và không còn revision-history action. |
