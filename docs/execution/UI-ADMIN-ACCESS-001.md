...
| **Giá mặt hàng** | Collection theo bảng giá và NCC đã chọn; không split màn hình | `#`, Mặt hàng (tên + mã hệ thống), Danh mục, Đơn vị, Đơn giá, VAT, MOQ, Lead time, Mặc định, Thao tác | Net price nếu sau này tách khác giá, mô tả, trạng thái, audit | Modal vừa 720–820px; price list + supplier khóa theo context, chọn item active chưa có trong bảng giá, một ô đơn giá, VAT, MOQ, lead, default, mô tả. |
...
| `/library?tab=6&pricingTab=prices` | `/api/vppprice/item-prices` theo `priceListId + supplierId`; lookup `/api/vpppricelist` và suppliers | Chỉ reload khi đủ hai context; DOM row thật bắt buộc; mã mặt hàng hệ thống/price/VAT/MOQ/lead thuộc route typed | `Tab_PriceLibrary` + `VppCollectionWorkspace` | Shared Collection/Frame/Toolbar CSS + pricing selectors trong `vpp-admin.css` |
