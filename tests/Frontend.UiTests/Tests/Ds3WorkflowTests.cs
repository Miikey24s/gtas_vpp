...
public sealed class Ds3WorkflowTests : TestBase, IAuthenticatedUiTest
{
    ...
    public async Task PeriodSettlement_UsesItemAndDepartmentViewsWithInlineSupplierSelection()
    {
        ...
        (await Page.Locator(".vpp-settlement-decision-cards:visible").CountAsync()).Should().Be(1);
        (await Page.Locator(".vpp-settlement-export-card:visible").CountAsync()).Should().Be(2);
