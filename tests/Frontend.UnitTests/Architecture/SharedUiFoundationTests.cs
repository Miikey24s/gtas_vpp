...
public sealed class SharedUiFoundationTests
{
    ...
    public void HistoryRoute_UsesAlignedPagedSummaryAndBoundedDetailDrawer()
    {
        ...
        Assert.Contains("AllowVirtualization=\"true\"", orderItemsSurface, StringComparison.Ordinal);
        Assert.Contains("IsHistoryDrawer ? 4 : 10", orderItemsSurface, StringComparison.Ordinal);
        Assert.Contains("vpp-history-detail-grid-header", orderItemsSurface, StringComparison.Ordinal);
        ...
        Assert.Contains(".rz-data-grid-data.has-vertical-overflow", orderItemsStyles, StringComparison.Ordinal);
        Assert.Contains("--vpp-history-detail-scrollbar-width", orderItemsStyles, StringComparison.Ordinal);
        Assert.Contains("position: absolute;", orderItemsStyles, StringComparison.Ordinal);
        ...
        Assert.Contains("root.querySelectorAll('.vpp-history-grid .rz-data-grid-data')", historyScript, StringComparison.Ordinal);
        Assert.Contains("if (surface.closest('.vpp-order-items-surface')) return;", historyScript, StringComparison.Ordinal);
        Assert.Contains("surface.offsetWidth - surface.clientWidth", orderItemsScript, StringComparison.Ordinal);
