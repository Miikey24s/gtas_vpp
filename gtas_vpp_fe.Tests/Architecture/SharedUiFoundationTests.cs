...
public sealed class SharedUiFoundationTests
{
    ...
    public void SidebarNavigation_UsesAppleMusicInspiredRowStates()
    {
        ...
        Assert.Contains("--vpp-sidebar-expanded-nav-overlap: 14px;", sidebarCss, StringComparison.Ordinal);
        Assert.Contains("--vpp-sidebar-collapsed-nav-overlap: 16px;", sidebarCss, StringComparison.Ordinal);
        Assert.Contains("border-block-end: 0 !important;", sidebarCss, StringComparison.Ordinal);
        Assert.Contains("logo -> first row and row -> row share exactly the same rhythm", sidebarCss, StringComparison.Ordinal);
