...
namespace gtas_vpp_be.Controllers
{
...

    {
        ...
        protected BaseGenericController(IBusinessService businessService)
        {
            ...
            Expression<Func<TModel, bool>>? matchSearch = null) where TModel : class
        {
            if (id.HasValue)
            {
                var dataById = await _businessService.BaseService<TModel>(EF_BASEMETHOD.EF_GetTAsync, true, matchId);
            ...
            {
                var dataBySearch = await _businessService.BaseService<TModel>(EF_BASEMETHOD.EF_GetTAsync, true, matchSearch);
                var dtoList = dataBySearch?.Adapt<List<TDto>>();
                return Ok(dtoList ?? new List<TDto>());
            }

            var allData = await _businessService.BaseService<TModel>(EF_BASEMETHOD.EF_GetTAsync, true);
        ...
        {
            var data = await _businessService.BaseService<TModel>(EF_BASEMETHOD.EF_GetTByIdAsync, true, Param: id);
        ...
        {
            var rs = await _businessService.BaseService<TModel>(EF_BASEMETHOD.EF_DeleteAsync, Param: id);
